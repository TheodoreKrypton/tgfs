...
class TestUploaderFromStream:
    ...
    async def test_stream_upload(self, mock_client):
        ...
        file_msg = FileMessageFromStream.new(stream=stream, size=total_size, name="stream_test.txt")
    ...
    async def test_stream_chunked_reading(self, mock_client):
        ...
        stream = self.create_test_stream(chunks)
        file_msg = FileMessageFromStream.new(stream=stream, size=total_size, name="chunked.txt")
...
class TestCreateUploaderFactory:
    ...
    def test_create_path_uploader(self, mock_tdlib):
        with tempfile.NamedTemporaryFile() as temp_file:
            ...
            file_msg = FileMessageFromPath.new(path=temp_file.name, name="path_test.txt")
    ...
    def test_create_stream_uploader(self, mock_tdlib):
        ...
        file_msg = FileMessageFromStream.new(stream=dummy_stream(), size=4, name="stream_test.txt")
