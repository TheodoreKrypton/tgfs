...
class TestTaskStore:
    ...
    @pytest.mark.asyncio
    async def test_update_task_progress_completed_sets_progress(self, task_store_instance):
        task_id = await task_store_instance.add_task(
            TaskType.UPLOAD, "/test/file.txt", "file.txt", size_total=1000
        )
    ...
    async def test_update_task_progress_speed_calculation_invalid_timestamp(self, task_store_instance):
        ...
        success = await task_store_instance.update_task_progress(task_id, size_delta=100)
        assert success is True
    ...
    @pytest.mark.asyncio
    async def test_update_task_progress_failed_status_with_no_size_total(self, task_store_instance):
        task_id = await task_store_instance.add_task(
            TaskType.UPLOAD, "/test/file.txt", "file.txt"  # No size_total
        )
        ...
        assert task.progress == 0.0  # Failed status sets progress to 0 when no size_total
    ...
    async def test_get_tasks_under_path_root(self, task_store_instance):
        ...
        await task_store_instance.add_task(
            TaskType.UPLOAD, "/file1.txt", "file1.txt"
        )
        await task_store_instance.add_task(
    ...
    async def test_cleanup_completed_tasks(self, task_store_instance):
        ...
        removed_count = await task_store_instance.cleanup_completed_tasks(max_age_hours=24)
        assert removed_count == 2  # old completed and old failed
        
        # Verify which tasks remain
        assert await task_store_instance.get_task(task_id1) is None  # Old completed removed
        assert await task_store_instance.get_task(task_id2) is not None  # Recent completed kept
        assert await task_store_instance.get_task(task_id3) is None  # Old failed removed
        assert await task_store_instance.get_task(task_id4) is not None  # In-progress kept
