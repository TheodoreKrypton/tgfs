...
from tgfs.reqres import (
    ...
    MessageResp, Document, GetMessagesResp
)
...
class TestTelethonAPI:
    ...
    def mock_client(self) -> AsyncMock:
        ...
        client.iter_download = AsyncMock()
        return client

    @pytest.fixture
    def telethon_api(self, mock_client: AsyncMock) -> TelethonAPI:
        return TelethonAPI(mock_client)

    @pytest.fixture
    def mock_chat(self) -> Mock:
        return Mock()

    @pytest.fixture
    def mock_message(self) -> Mock:
        message = Mock(spec=tlt.Message)
        message.id = 12345
    ...
    @pytest.fixture
    def mock_document_message(self) -> Mock:
        document = Mock(spec=tlt.Document)
        document.size = 1024
        document.id = 98765
        ...
        media = Mock(spec=tlt.MessageMediaDocument)
    ...
    @pytest.mark.asyncio
    async def test_get_messages_success_with_cache(self, telethon_api, mock_chat, mock_message):
        ...
        with patch('tgfs.telegram.impl.telethon.message_cache_by_id') as mock_cache:
    ...
    @pytest.mark.asyncio
    async def test_get_messages_with_cached_messages(self, telethon_api, mock_chat):
        req = GetMessagesReq(chat=mock_chat, message_ids=(12345,))
    ...
    def test_transform_messages_with_none_message(self):
        mock_msg = Mock(spec=tlt.Message)
        mock_msg.id = 123
    ...
    def test_transform_messages_with_empty_document(self):
        empty_doc = Mock(spec=tlt.DocumentEmpty)
        media = Mock(spec=tlt.MessageMediaDocument)
    ...
    @pytest.mark.asyncio
    async def test_send_text(self, telethon_api, mock_chat):
    ...
    @pytest.mark.asyncio
    async def test_edit_message_text(self, telethon_api, mock_chat):
    ...
    @pytest.mark.asyncio
    async def test_edit_message_media(self, telethon_api, mock_chat):
        mock_edited_message = Mock()
    ...
    @pytest.mark.asyncio
    async def test_search_messages(self, telethon_api, mock_chat, mock_message):
        total_list = TotalList([mock_message])
        telethon_api._client.get_messages.return_value = total_list
        
        req = SearchMessageReq(chat=mock_chat, search="test query")
        
        with patch('tgfs.telegram.impl.telethon.message_cache_by_search') as mock_cache:
    ...
    @pytest.mark.asyncio
    async def test_search_messages_with_cache(self, telethon_api, mock_chat):
    ...
    @pytest.mark.asyncio
    async def test_send_big_file(self, telethon_api, mock_chat):
        mock_sent_message = Mock(spec=tlt.Message)
    ...
    @pytest.mark.asyncio
    async def test_send_small_file(self, telethon_api, mock_chat):
        mock_sent_message = Mock(spec=tlt.Message)
...
class TestSession:
    ...
    def test_get_existing_session(self, session, session_file_path):
        ...
        with patch('telethon.sessions.StringSession.__init__', return_value=None):
            result = session.get()
...
class TestLoginAsAccount:
    ...
    def mock_config(self) -> Mock:
        ...
        config.telegram.account = Mock(spec=AccountConfig)
    ...
    @pytest.mark.asyncio
    async def test_login_as_account_no_account_config(self):
        config = Mock(spec=Config)
        config.telegram = Mock(spec=TelegramConfig)
    ...
    @pytest.mark.asyncio
    async def test_login_as_account_with_existing_session(self, mock_config):
        mock_session_string = Mock(spec=StringSession)
        mock_client = Mock(spec=TelegramClient)
        mock_user = Mock(spec=tlt.User)
        mock_user.username = "testuser"
        
        with patch('tgfs.telegram.impl.telethon.Session') as MockSession, \
             patch('tgfs.telegram.impl.telethon.TelegramClient') as MockTelegramClient:
    ...
    @pytest.mark.asyncio
    async def test_login_as_account_new_session_success(self, mock_config):
        mock_client = Mock(spec=TelegramClient)
        mock_user = Mock(spec=tlt.User)
        ...
        with patch('tgfs.telegram.impl.telethon.Session') as MockSession, \
            ...
            MockTelegramClient.return_value = mock_client
            mock_client.connect = AsyncMock()
            mock_client.send_code_request = AsyncMock(return_value=mock_sms_req)
    ...
    @pytest.mark.asyncio
    async def test_login_as_account_with_2fa(self, mock_config):
        mock_client = Mock(spec=TelegramClient)
        mock_user = Mock(spec=tlt.User)
        ...
        with patch('tgfs.telegram.impl.telethon.Session') as MockSession, \
             patch('tgfs.telegram.impl.telethon.TelegramClient') as MockTelegramClient, \
             patch('builtins.input') as mock_input, \
             patch('tgfs.telegram.impl.telethon.getpass') as mock_getpass:
            ...
            mock_client.sign_in = AsyncMock(side_effect=[SessionPasswordNeededError("Mock request"), None])
            mock_client.get_me = AsyncMock(return_value=mock_user)
            mock_client.session = Mock()
            mock_client.session.save.return_value = "new_session_string"
    ...
    @pytest.mark.asyncio
    async def test_login_as_account_sign_in_error(self, mock_config):
        mock_client = Mock(spec=TelegramClient)
        mock_sms_req = Mock()
        mock_sms_req.phone_code_hash = "test_hash"
        ...
        with patch('tgfs.telegram.impl.telethon.Session') as MockSession, \
            ...
            mock_client.connect = AsyncMock()
            mock_client.send_code_request = AsyncMock(return_value=mock_sms_req)
            mock_client.sign_in = AsyncMock(side_effect=Exception("Sign in failed"))
    ...
    @pytest.mark.asyncio
    async def test_login_as_account_no_username(self, mock_config):
        mock_client = Mock(spec=TelegramClient)
        mock_user = Mock(spec=tlt.User)
        ...
        with patch('tgfs.telegram.impl.telethon.Session') as MockSession, \
             patch('tgfs.telegram.impl.telethon.TelegramClient') as MockTelegramClient, \
             patch('tgfs.telegram.impl.telethon.logger') as mock_logger:
            
            mock_session_instance = MockSession.return_value
            mock_session_instance.get.return_value = Mock(spec=StringSession)
...
class TestLoginAsBots:
    ...
    def mock_config(self) -> Mock:
        ...
        config.telegram.api_id = 12345
        config.telegram.api_hash = "test_api_hash"
        config.telegram.bot = Mock(spec=BotConfig)
    ...
    @pytest.fixture
    def mock_config_single_token(self) -> Mock:
        config = Mock(spec=Config)
        config.telegram = Mock(spec=TelegramConfig)
        config.telegram.api_id = 12345
        config.telegram.api_hash = "test_api_hash"
    ...
    @pytest.mark.asyncio
    async def test_login_as_bots_multiple_tokens(self, mock_config):
        mock_clients = [Mock(spec=TelegramClient), Mock(spec=TelegramClient)]
        mock_users = [Mock(spec=tlt.User), Mock(spec=tlt.User)]
        ...
        with patch('tgfs.telegram.impl.telethon.Session') as MockSession, \
            ...
            for i, client in enumerate(mock_clients):
                ...
                client.get_me = AsyncMock(return_value=mock_users[i])
                client.session = Mock()
                client.session.save.return_value = f"session_string_{i}"
                mock_session_instances[i].save_multibot = Mock()
    ...
    @pytest.mark.asyncio
    async def test_login_as_bots_single_token(self, mock_config_single_token):
        mock_client = Mock(spec=TelegramClient)
        mock_user = Mock(spec=tlt.User)
        ...
        with patch('tgfs.telegram.impl.telethon.Session') as MockSession, \
            ...
            mock_session_instance.save_multibot = Mock()
    ...
    @pytest.mark.asyncio
    async def test_login_as_bots_with_existing_session(self, mock_config_single_token):
        mock_client = Mock(spec=TelegramClient)
        mock_user = Mock(spec=tlt.User)
        mock_user.username = "existingbot"
        existing_session = Mock(spec=StringSession)
        ...
        with patch('tgfs.telegram.impl.telethon.Session') as MockSession, \
            ...
            MockTelegramClient.return_value = mock_client
            mock_client.connect = AsyncMock()
            mock_client.start = AsyncMock()  # Should not be called
            mock_client.get_me = AsyncMock(return_value=mock_user)
    ...
    @pytest.mark.asyncio
    async def test_login_as_bots_no_username(self, mock_config_single_token):
        mock_client = Mock(spec=TelegramClient)
        mock_user = Mock(spec=tlt.User)
        mock_user.username = None
