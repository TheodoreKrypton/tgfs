...
class TestTelethonAPI:
    ...
    @pytest.mark.asyncio
    async def test_get_messages_raises_error_on_invalid_response(
        self, telethon_api, mock_chat
    ):
        ...
        with pytest.raises(
            TechnicalError, match="Unexpected response type from get_messages"
        ):
            await telethon_api.get_messages(req)

    @pytest.mark.asyncio
    async def test_get_messages_success_with_cache(
        self, telethon_api, mock_chat, mock_message, mocker
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_messages_with_cached_messages(
        self, telethon_api, mock_chat, mocker
    ):
        req = GetMessagesReq(chat=mock_chat, message_ids=(12345,))
    ...
    async def test_send_big_file(self, telethon_api, mock_chat, mocker):
        ...
        req = SendFileReq(
            chat=mock_chat, file=uploaded_file, name="big_file.zip", caption="Big file"
        )
    ...
    async def test_send_big_file_invalid_response(self, telethon_api, mock_chat):
        ...
        uploaded_file = UploadedFile(id=123, parts=10, name="test.txt")
        req = SendFileReq(
            chat=mock_chat, file=uploaded_file, name="test.txt", caption="Test"
        )

        with pytest.raises(
            TechnicalError, match="Unexpected response type from send_file"
        ):
    ...
    async def test_download_file_message_without_document(
        self, telethon_api, mock_chat, mock_message
    ):
    ...
    @pytest.mark.asyncio
    async def test_download_file_invalid_range(
        self, telethon_api, mock_chat, mock_document_message
    ):
        total_list = TotalList([mock_document_message])
        telethon_api._client.get_messages.return_value = total_list
...
class TestSession:
    ...
    def test_save_multibot_existing_file_as_directory_raises_error(self, tmp_path):
        ...
        with pytest.raises(
            Exception, match="is a session file which only supports one bot session"
        ):
            session.save_multibot("test_session_string")
...
class TestLoginAsAccount:
    ...
    async def test_login_as_account_with_2fa(self, mock_config, mocker):
        ...
        mock_client.sign_in = mocker.AsyncMock(
            side_effect=[SessionPasswordNeededError("Mock request"), None]
        )
        mock_client.get_me = mocker.AsyncMock(return_value=mock_user)
        mock_client.session = mocker.Mock()
        mock_client.session.save.return_value = "new_session_string"
...
class TestLoginAsBots:
    ...
    async def test_login_as_bots_multiple_tokens(self, mock_config, mocker):
        mock_clients = [
            mocker.Mock(spec=TelegramClient),
            mocker.Mock(spec=TelegramClient),
        ]
    ...
    @pytest.mark.asyncio
    async def test_login_as_bots_with_existing_session(
        self, mock_config_single_token, mocker
    ):
        mock_client = mocker.Mock(spec=TelegramClient)
        mock_user = mocker.Mock(spec=tlt.User)
