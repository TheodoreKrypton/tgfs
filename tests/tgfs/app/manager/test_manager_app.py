...
class TestManagerApp:
    ...
    async def test_get_tasks_all(self, manager_app, mocker):
        ...
        mock_tasks = [mocker.Mock(to_dict=mocker.Mock(return_value={"id": "1", "path": "/test"}))]
    ...
    async def test_get_tasks_filtered_by_path(self, manager_app, mocker):
        mock_task_store = mocker.patch('tgfs.app.manager.app.task_store')
        mock_tasks = [mocker.Mock(to_dict=mocker.Mock(return_value={"id": "2", "path": "/specific"}))]
    ...
    async def test_get_task_found(self, manager_app, mocker):
        mock_task_store = mocker.patch('tgfs.app.manager.app.task_store')
        mock_task = mocker.Mock(to_dict=mocker.Mock(return_value={"id": "task123", "status": "running"}))
    ...
    @pytest.mark.asyncio
    async def test_get_telegram_message_no_document(self, mock_client, mock_config, mocker):
        mock_message = mocker.Mock()
        mock_message.message_id = 456
    ...
    @pytest.mark.asyncio
    async def test_import_telegram_message_success(self, mock_client, mock_config, mocker):
        # Mock the message response
